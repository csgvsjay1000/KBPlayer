...
-(void)play{
    _playingState = KBPlayerPlayingStatePlaying;
    if (_is) {
...
-(BOOL)seekToTime:(CGFloat)toTime{
    if (_is && _is->video_stream>=0 && _is->audio_stream>=0 && toTime>0 && toTime<_duration ) {
        toTime = toTime*AV_TIME_BASE;
        packet_queue_destroy(&_is->audioq);
        packet_queue_destroy(&_is->videoq);
        packet_queue_init(&_is->audioq);
        packet_queue_init(&_is->videoq);
        
        if(av_seek_frame(_is->ic, -1, toTime, AVSEEK_FLAG_ANY)<0){
            NSLog(@"seek 失败");
            return NO;
        };
...
-(void)read_thread{
    ...
    if (ic->duration != AV_NOPTS_VALUE) {
        _duration = _is->ic->duration /AV_TIME_BASE ;
        
        if (self.playerDelegate) {
            [self.playerDelegate refreshDuration];
        }
    }
    for (; ; ) {
        if (quit) {
        ...
        if (_playingState == KBPlayerPlayingStatePause) {
            usleep(10*1000);
            continue;
        }
        ...
        if (av_read_frame(_is->ic, packet)>=0) {
            ...
            if (self.currentDuration >= _duration && _duration >0) {
                _playerState = KBPlayerStateParseFinshed;
                if ([self playFinshed]) {
                    break;
                }
            }
...
-(int)synchronize_audio:(VideoState *)is samples:(short *)samples samples_size:(int)samples_size pts:(double)pts {
    ...
    diff = [self get_audio_clock] - ref_clock;
...
static void AQueueOutputCallback(
    ...
    KBPlayer11_0 *vc = (__bridge KBPlayer11_0 *)inUserData;
    if (vc) {
...
-(void)audio_stream_component_open:(int)stream_index{
    ...
    _is->audio_src_freq = _is->audio_tgt_freq = codecCtx->sample_rate;
    _is->audio_src_fmt = _is->audio_tgt_fmt = AV_SAMPLE_FMT_S16;
    _is->audio_src_channel_layout = _is->audio_tgt_channel_layout = AV_CH_LAYOUT_STEREO;
    _is->audio_src_channels = _is->audio_tgt_channels = av_get_channel_layout_nb_channels(_is->audio_tgt_channel_layout);
...
-(BOOL)playFinshed{
    if (_playerState == KBPlayerStateParseFinshed) {
        sleep(2);
        return YES;
    }
    return NO;
...
-(CGFloat)currentDuration{
    if (_is) {
        _currentDuration = _is->video_clock;
        return _is->video_clock;
